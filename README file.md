...
**Frontend (Netlify):** https://prismdashboard.netlify.app

> **Important:** The live demo requires the backend to be running locally on your machine. The frontend is hosted on Netlify but connects to `localhost:8000` for all ML predictions, history, and analytics.
>
> To use the live demo:
> 1. Clone this repo
> 2. Start the backend: `python -m uvicorn main:app --reload --port 8000`
> 3. Open https://prismdashboard.netlify.app in your browser
> 4. The dashboard will connect to your local backend automatically

If the backend is not running, the frontend will load but all data will show as offline (dashes and empty tables). This is expected — the ML model runs on Python and cannot be hosted for free on a static hosting platform like Netlify.
...
| Layer | Platform | URL |
|---|---|---|
| Frontend | Netlify (free) | https://prismdashboard.netlify.app |
| Backend | Local machine | http://localhost:8000 |
| API Docs | FastAPI Swagger | http://localhost:8000/docs |

To make the backend permanently available online for all users, deploy it to a platform like Render, Railway, or AWS EC2 and update the `VITE_API_URL` environment variable in Netlify to point to the deployed backend URL.
